direccionesModulo = (function () {
  ...
  function agregarDireccionYMostrarEnMapa (direccion, ubicacion, titulo) {
    that = this
    ...
    marcadorModulo.mostrarMiMarcador(ubicacion, titulo)
  }
  ...
  function calcularYMostrarRutas () {
        ...
        var pasosIntermediosSelect = document.getElementById('puntosIntermedios');
        for (var i = 0; i < pasosIntermediosSelect.length; i++) {
          if (pasosIntermediosSelect.options[i].selected) {
            pasosIntermedios.push({
              location: pasosIntermediosSelect[i].value,
              stopover: true
            });
          }
        }
        ...
        if (status == 'OK') {
          ...
          var direccionesPanel = document.getElementById('directions-panel');
          direccionesPanel.innerHTML = '';
        }
      })
  }
